import argparse
import os
import shutil

import pandas as pd
from sklearn.model_selection import train_test_split
from tqdm import tqdm


def find_paired_images(df):
    pairs = []
    for (_, _), group in df.groupby(["study_id", "laterality"]):
        cc_rows = group[group["view_position"] == "CC"]
        mlo_rows = group[group["view_position"] == "MLO"]
        if len(cc_rows) == 1 and len(mlo_rows) == 1:
            cc_row = cc_rows.iloc[0]
            mlo_row = mlo_rows.iloc[0]
            if cc_row["split"] == mlo_row["split"]:
                pairs.append((cc_row, mlo_row))
            else:
                print(f"Warning: mismatched splits for {cc_row['study_id']}-{cc_row['laterality']}")
    return pairs


def create_output_structure(output_dir):
    for view in ["CC", "MLO"]:
        for split in ["train", "val", "test"]:
            os.makedirs(os.path.join(output_dir, view, split), exist_ok=True)


def copy_paired_images(pairs, split_name, source_image_dir, output_dir):
    for cc_row, mlo_row in tqdm(pairs, desc=f"Copying {split_name}"):
        study_id = cc_row["study_id"]
        laterality = cc_row["laterality"]
        base_name = f"{study_id}_{laterality}"
        cc_src = os.path.join(source_image_dir, study_id, f"{cc_row['image_id']}_resized.png")
        mlo_src = os.path.join(source_image_dir, study_id, f"{mlo_row['image_id']}_resized.png")
        cc_dst = os.path.join(output_dir, "CC", split_name, f"{base_name}_CC.png")
        mlo_dst = os.path.join(output_dir, "MLO", split_name, f"{base_name}_MLO.png")

        if os.path.exists(cc_src):
            shutil.copy2(cc_src, cc_dst)
        else:
            print(f"Warning: source file not found: {cc_src}")
        if os.path.exists(mlo_src):
            shutil.copy2(mlo_src, mlo_dst)
        else:
            print(f"Warning: source file not found: {mlo_src}")


def parse_args():
    parser = argparse.ArgumentParser(description="Prepare paired VinDr-Mammo CC/MLO images.")
    parser.add_argument("--annotations_csv", required=True, help="VinDr breast-level annotations CSV.")
    parser.add_argument("--source_image_dir", required=True, help="Directory with resized image study subfolders.")
    parser.add_argument("--output_dir", required=True, help="Output paired dataset directory.")
    parser.add_argument("--val_split", type=float, default=0.05, help="Validation split from training pairs.")
    parser.add_argument("--seed", type=int, default=42)
    return parser.parse_args()


def main():
    args = parse_args()
    df = pd.read_csv(args.annotations_csv)
    pairs = find_paired_images(df)
    train_pairs = [(cc, mlo) for cc, mlo in pairs if cc["split"] == "training"]
    test_pairs = [(cc, mlo) for cc, mlo in pairs if cc["split"] == "test"]

    if train_pairs:
        train_pairs, val_pairs = train_test_split(
            train_pairs, test_size=args.val_split, random_state=args.seed
        )
    else:
        val_pairs = []

    create_output_structure(args.output_dir)
    copy_paired_images(train_pairs, "train", args.source_image_dir, args.output_dir)
    copy_paired_images(val_pairs, "val", args.source_image_dir, args.output_dir)
    copy_paired_images(test_pairs, "test", args.source_image_dir, args.output_dir)

    print("Dataset preparation complete.")
    print(f"Output directory: {args.output_dir}")
    print(f"Train pairs: {len(train_pairs)}")
    print(f"Val pairs: {len(val_pairs)}")
    print(f"Test pairs: {len(test_pairs)}")


if __name__ == "__main__":
    main()
