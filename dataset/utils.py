...
def get_brief_prompt(prompt):
    ...
    output_sentences = [
        ...
        if sentence.strip() and not any(pattern in sentence for pattern in sent_to_remove)
    ]
...
def get_gray_imgs(img_path, scale, transform=None, multiscale=False, return_orig_img=False):
...
def get_imgs(img_path, scale=None, transform=None, multiscale=False, return_orig_img=False, return_mask=False):
    x = cv2.imread(str(img_path), 0)
    # tranform images
    if scale:
        x = resize_img(x, scale)
    ...
    if return_orig_img:
        ...
        img_rescale = img.clone()
        img_rescale = (img_rescale - img_rescale.min()) / (img_rescale.max() - img_rescale.min())
        img_rescale = img_rescale.clamp(0, 1)
        foreground_mask = (img_rescale).mean(0) > img_rescale.min() + 4e-2
        foreground_mask = foreground_mask.unsqueeze(0).to(img.device).long() # (1, h, w)
        return img, foreground_mask
    else:
        return img

def get_breast_area(img):
    H, W = img.shape[-2:]
    total_area = H * W
    breast_mask = img > 0
    breast_area = breast_mask.sum()
    breast_area_ratio = breast_area / total_area
    return breast_area_ratio

def get_tokenizer(llm_type, old_tokenizer=False):
    if llm_type == 'gpt':
        ...
        if old_tokenizer:
            ...
            tokenizer.add_special_tokens({
                ...
                'additional_special_tokens': ['<|keytext|>',],
            })
    elif llm_type == 'llama':
        ...
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-7b-hf", token=MY_API_TOKEN,
                                                        padding_side='right')
        tokenizer.add_special_tokens({
            'pad_token': '<pad>',
            'mask_token': '<mask>',
        ...
        tokenizer = AutoTokenizer.from_pretrained("meta-llama/Meta-Llama-3-8B", token=MY_API_TOKEN)
        tokenizer.add_special_tokens({
            'pad_token': '<|pad_text|>',
            'mask_token': '<|mask_text|>',
            'sep_token': '<|separate_of_text|>',
            'additional_special_tokens': ['<|keyword_of_text|>',],
        })
    else:
