...
def enhance_contrast(image, method: str = 'histeq'):
    """Enhance contrast of an image.

    Args:
        image: PIL Image or NumPy array.
        method: Normalization method to use. One of:
            - 'histeq': Global histogram equalization (default).
    ...
    if method == 'histeq':
        ...
        if img_max > img_min:
            enhanced_image = ((image.astype(np.float32) - img_min) / (img_max - img_min) * 255).astype(np.uint8)
        else:
        ...
        raise ValueError(f"Unknown contrast enhancement method: '{method}'. "
                         "Choose from 'histeq', 'minmax', or 'clahe'.")
...
class ContrastEnhance(object):
    def __init__(self, method: str = 'histeq'):
        """Args:
            method: Contrast enhancement method. One of 'histeq', 'minmax', or 'clahe'.
        """
        super().__init__()
        self.method = method
    ...
    def __process__(self, x):
        if isinstance(x, Image.Image):
            x = np.array(x)
        x = enhance_contrast(x, method=self.method)
        return x
...
class DataTransforms(object):
    def __init__(self, is_train: bool = True, img_size: int = 256, crop_size: int = 224,
                 align_orientation: bool = False, remove_text: bool = False):
        if is_train:
            data_transforms = [
...
class DataFixedTransforms(object):
    def __init__(self, is_train: bool = True, img_size: int = 256, crop_size: int = 224,
                 align_orientation: bool = True, remove_text: bool = True):
        if is_train:
            data_transforms = [
                OtsuCut(align_orientation, remove_text),
                transforms.Resize((img_size, img_size)),
...
class DetectionDataTransforms(object):
    def __init__(self, is_train: bool = True, image_size: int = 224, 
                 crop_size: int = 224, jitter_strength: float = 1.):
        if is_train:
...
class Moco2Transform(object):
    def __init__(self, is_train: bool = True, img_size: int = 256, crop_size: int = 224,
                 align_orientation: bool = False, remove_text: bool = False) -> None:
        if is_train:
        ...
        if is_train:
            self.data_transforms = transforms.Compose(
                [
                    ...
                    transforms.RandomApply([transforms.GaussianBlur((5, 5), [0.1, 2.0])], p=0.4),
                    transforms.ToTensor(),
                    transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
                ]
            )
...
class RandomWordDeletion(object):
    ...
    def __init__(self, p=0.1, max_deletion=10, punctuations='!"#$%&\'()*+,-./:;<=>?@[\\]^_`{|}~'):
        self.p = p
...
class TextTransform(object):
    ...
    def __init__(self, is_train: bool = True, 
                 ...
                 random_swap_prob: float = 0., 
                 random_deletion_prob: float = 0., 
        ...
        remove_stop_word = lambda x: ' '.join([word for word in x.split() if word.lower() not in self.stop_words])
        eda = EDA()
        # This format only uses 1 augmenter at a time, replace it with a list of augmenters
        if bos_token is not None:
            self.preprocess_func = lambda x: x.replace(self.bos_token, '').replace(self.eos_token, '').replace(self.stop_token, '.')
            self.postprocess_func = lambda x: f'{self.bos_token} {x} {self.eos_token}'.replace('.', self.stop_token)
        else:
